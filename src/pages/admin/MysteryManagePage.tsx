...
export default function MysteryManagePage({ user, onBack }: Props) {
  ...
  const statusLabel = (s: MysterySession) => {
    const now = new Date()
    if (s.status === 'completed') return { text: '已完成', color: '#10b981', bg: 'var(--color-green-50)', icon: <CheckCircle2 className="w-4 h-4" /> }
            ...
            {!generatedUrl ? (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <button
                  ...
                  style={{ background: 'linear-gradient(135deg,var(--brand),var(--brand-dark))', opacity: (!storeId.trim() || creating) ? 0.5 : 1 }}
                >
