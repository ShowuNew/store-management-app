...
import { ClipboardList, AlertTriangle, BarChart2, ShieldCheck, RefreshCw, LogOut, UserSearch, Building2, FileSearch, MousePointerClick, MapPin, Settings2 } from 'lucide-react'
...
export default function AdminDashboard({ user, onNavigate, onLogout }: Props) {
  ...
  const quickLinks = [
    ...
    { page: 'admin-feature-clicks' as Page, label: '功能點擊排行', desc: '各功能進入次數，可跨門市跨日期篩選', icon: <MousePointerClick   className="w-5 h-5" />, color: '#0ea5e9', bg: '#f0f9ff' },
    { page: 'supervisor-import'  as Page, label: '輪區匯入',   desc: '每日擔當轄區店鋪對應檔匯入',           icon: <MapPin     className="w-5 h-5" />, color: '#16a34a', bg: '#f0fdf4' },
    { page: 'admin-permissions'  as Page, label: '功能權限設定', desc: '各群組（SAT/全管家/全家人/後台）功能開關', icon: <Settings2  className="w-5 h-5" />, color: '#6366f1', bg: '#eef2ff' },
  ]
